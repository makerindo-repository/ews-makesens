                    ...
                    <form action="{{ route('user.update', $user->id) }}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                            <div class="md:col-span-2">
                                <x-input-label for="profile_picture">{{ __('Foto Profil (Tidak wajib)') }}</x-input-label>
                                <div class="mt-1">
                                    <div class="relative">
                                        <!-- Preview Container -->
                                        <div id="imagePreviewContainer"
                                            ...
                                            <div id="imagePreviewContent" class="text-center">
                                                <i
                                                    class="fa-solid fa-cloud-upload-alt mx-auto text-5xl text-gray-400 mb-2"></i>
                            ...
                            <div>
                                ...
                                <x-text-input id="email" class="block mt-1 w-full rounded-xl bg-gray-100"
                                    type="email" name="email" :value="$user->email" required autofocus
                                    autocomplete="email" />
                                <x-input-error :messages="$errors->get('email')" class="mt-2" />
                            </div>
                            <div>
                                <x-input-label for="password" :value="__('Password (Tidak wajib)')" />
                            ...
                            <div>
                                ...
                                <x-text-input id="phone" class="block mt-1 w-full rounded-xl bg-gray-100"
                                    type="tel" name="phone" :value="$user->phone" required autofocus
                                    autocomplete="phone" />
                                ...
                                <div class="flex gap-3 mt-3">
                                    <x-input-label for="man">
                                        <input id="man" class="bg-gray-100" type="radio" name="gender"
                                            value="l" required autofocus autocomplete="gender" @checked($user->gender == 'l')/>
                                        {{ __('Laki-laki') }}
                                    </x-input-label>
                                    <x-input-label for="woman">
                                        <input id="woman" class="bg-gray-100" type="radio" name="gender"
                                            value="p" required autofocus autocomplete="gender" @checked($user->gender == 'p')/>
                                        {{ __('Perempuan') }}
                                    </x-input-label>
                                </div>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea name="address" id="address" class="block mt-1 w-full rounded-xl bg-gray-100" rows="3" required>{{ $user->address }}</textarea>
